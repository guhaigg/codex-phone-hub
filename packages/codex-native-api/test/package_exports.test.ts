...
test('package metadata and root entrypoint keep a stable public boundary', () => {
  ...
  assert.equal(packageJson.name, '@codex-phone-hub/codex-native-api');
...
test('daemon layout resolves platform-specific service paths', () => {
  ...
  assert.equal(darwin.envFile, path.resolve('/tmp/darwin-home', '.config/codex-native-api/service.env'));
  assert.equal(darwin.launchdPlistPath, path.resolve('/tmp/darwin-home', 'Library/LaunchAgents/com.codexbridge.codex-native-api.plist'));
  ...
  assert.equal(linux.systemdUnitPath, path.resolve('/tmp/linux-home', '.config/systemd/user/codex-native-api.service'));
